...
public class PostgresQueuePickTaskDao implements QueuePickTaskDao {
    ...
    private Map<String, Object> pickTaskSqlPlaceholders;
     ...
     */
    public PostgresQueuePickTaskDao(@Nonnull JdbcOperationsSubset jdbcTemplate,
        ...
        pickTaskSqlPlaceholders = new HashMap<>();
        pickTaskSqlPlaceholders.put("queueName", queueLocation.getQueueId().asString());
        pickTaskSqlPlaceholders.put("retryInterval", failureSettings.getRetryInterval().getSeconds());
        failureSettings.registerObserver((oldValue, newValue) -> {
            pickTaskSql = createPickTaskSql(queueLocation, newValue);
            pickTaskSqlPlaceholders = new HashMap<>();
            pickTaskSqlPlaceholders.put("queueName", queueLocation.getQueueId().asString());
            pickTaskSqlPlaceholders.put("retryInterval", newValue.getRetryInterval().getSeconds());
        });
    }
